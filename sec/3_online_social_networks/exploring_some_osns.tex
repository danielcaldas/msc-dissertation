...
have their apps embedded in the Facebook platform understand their audience and the performance of their apps, etc. \citep{fbproducts}.\\
...
from Facebook such as posts, photos, videos, pages etc. According to \citep{fbgapi}, the common scenarios for using the Graph \gls{api}
...
brands, to teens, musicians and anyone with a creative passion."} \citep{instabout}
\end{quote}
...
\indent According to \citep{instabout}, since
...
perform 30 requests per hour to getting users' relationships data. \citep{instadev}

%% ---------------------------------------------- Exploring Some OSNs
%% -------------------------------------------------- LinkedIn
\subsection{LinkedIn}

Moving on to the next \gls{osn} we now have LinkedIn. According to \citep{linkabout}, LinkedIn was launched officially on May 5 of 2003, and by the end of
...
LinkedIn provides a REST \gls{api} \citep{linkapi}, but still similarly to the \glspl{osn} we been studying its very limited. In what concerns to data retrieval LinkedIn only allows
...
Horst Fickenscher, ResearchGate today has more than 11+ million members. We strive to help them make progress happen faster."} \citep{rgate}
\end{quote}

...
\indent Along the chapter we will not only list results but show them as well: in some of the following sections we present a set of Socii images that depict the implemented functionalities and how to use them. Then, we present a case study, were we analyze a real network and derive some different conclusions that well demonstrate some of the use cases of Socii.\\
...
In Table \ref{table:featuresocii} we present a list of the implemented requirements, these refer to the requirements in chapter 6. All the requirements are \textbf{MUST} requirements unless otherwise is stated.

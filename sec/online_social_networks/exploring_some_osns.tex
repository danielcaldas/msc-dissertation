...
\begin{figure}[h!]
  ...
  \includegraphics[width=1.18\textwidth]{img/facebook-domain-model-opt.jpg}
...
\indent Unlike many other social networks that have private or semi-public profiles with restrict policies concerning to exterior acess to information within the network (examples of this kind of \glspl{osn} may be LinkedIn or Facebook),
Twitter default settings are public, making tweets spread more effectivily accross all social media, this particularity makes Twitter one of the most \textit{"barrier-free}" \glspl{osn}. Of course that despite unregistred people may read tweets
...
\begin{figure}[h!]
  \hspace*{-1in}
  \includegraphics[width=1.10\textwidth]{img/twitter-domain-model.jpg}
...
concerning to data properties and relationship complecity, Twitter has some very semantically strong features that brand the platform, being those
features used among other well known \glspl{osn} such as Facebook. We are refering specifically the hastag ((\textit{HashTag} box) and the (\textit{TwitterHandle} box),
but let us first introduce some of the more basic features.\\

\indent As usual in these platforms Twitter's users posses a user profile
...
other users the user is following, and number of likes the user has obtained across all his or her tweets (\textit{Followers}, \textit{Tweets}, \textit{Following}, \textit{Likes} boxes respectivelly).\\
...
be \textit{retweeted}. Now back to hastags and twitter handles. Hashtags is in some way how these caos of unstructured tweets gain some semantic
value, in order to agroup tweets according to a specific matter. Hashtags may in many cases be mesleading, because users
start to adopt hashtag to express sentiments or simply describing a tweet pushing back to the already mencioned unstructed caos.
Twitter handles are the same as tags in Facebook they serve as a mean to mark a specific twitter user, a twitter handle may be invoqued in a comment or
in a tweet leading directily to the respective user's twitter profile.

\subsubsection*{Twitter API}
Simillarly to other \glspl{osn}, twitter provides a REST API to fetch users data such as profile data, tweets or user's followers. Restrictions are felt
...
KONECT(http://konect.uni-koblenz.de/) aim to provide large data sets of networks collected online so that researchers may
...
analysis even for real time data analysis systems that may in a more imature phase of the project benefit from this data sets.
...
of Web Science and Technologies at the University of Koblenz–Landau."} \cite{rgate}
\end{quote}

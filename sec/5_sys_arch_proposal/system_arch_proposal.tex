...
\begin{itemize}
    \item \textbf{Availability (Desktop or Web)} - Whether the tool available trough a desktop application or a web application;
    ...
    \item \textbf{Network Editor (Yes, No)} - Whether the tool allows network editing, such feature allows adding nodes and edges to existing network or event creating new networks from scratch;
...
\hline
\textbf{Tool} & \textbf{Availability} & \textbf{Complexity} & \textbf{Performance} & \textbf{Network Editor} & \textbf{\glspl{osn} Integration} & \textbf{Contextual Analysis}\\
\hline
...
\hline
\textbf{SocNetV} & Desktop & Low & Medium & Yes & No\footnote{SocNetV has a feature that allows the user to launch a \textit{web spider} that navigates trough web sites building a network representative of the links between the sites, but the \textit{spider} is not content aware, it blindly builds a network without context} & No\\
\hline
\textbf{Our system} & Web & Low & Low & No & Yes & Yes\\
\hline
\end{tabular}
